...
def extract_value(string: str, key: str) -> float:
    ...
    match = re.search(rf"{key}=([\d.]+)", string)
    if match:
        value = match.group(1)
    return float(value)
...
def calc_rte(df: pd.DataFrame) -> float:
    ...
    p = df["power_sim"]
    e_ch = p[p > 0].abs().sum() * (1 / 60)  # Wh
    e_dch = p[p < 0].abs().sum() * (1 / 60)  # Wh

    delta_soc = df["soc_sim"].iloc[-1] - df["soc_sim"].iloc[0]
    delta_e = delta_soc * 180e3  # Wh

    return abs(e_dch) / (e_ch - delta_e)
...
def calc_imbalance_dch(df: pd.DataFrame) -> float:
    df["imb"] = -(df["power_opt"] - df["power_sim"]) * (1 / 60)  # Wh
    # negation since positive power is charging
    return df.loc[df.imb > 0, "imb"].sum()  # BESS under-supply


def calc_imbalance_ch(df: pd.DataFrame) -> float:
    df["imb"] = -(df["power_opt"] - df["power_sim"]) * (1 / 60)  # Wh
    # negation since positive power is charging
    ...
    for id, df in res.items():
        if "LP" in id:
            data = dict(
                ...
                imb_ch=calc_imbalance_ch(df),
                imb_dch=calc_imbalance_dch(df),
            )
